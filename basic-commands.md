...
```bash
docker build /path/to/dockerfile -t <YOUR_TAG>
```
...
```bash
docker create network <UR_NETWORK_NAME>
```
...
```bash
docker run --name <UR_CUSTOM_IMAGE_NAME> --network <UR_NETWORK> \
-v <UR_VOLUME> -e <ENV_VAR> <IMAGE_NAME> 
```
...
```bash
docker start <UR_CONTAINER_NAME>
```
...
```bash
docker cp <FILE_IN_HOST> <UR_CONTAINER>:<PATH_IN_CONTAINER>
```
